...
import { Attendance, type IAttendanceDocument } from "../models/Attendance.model.js";
...
export const listAttendance = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (req.query.from || req.query.to) {
    filter.date = {};
    if (req.query.from) (filter.date as Record<string, string>).$gte = String(req.query.from);
    if (req.query.to) (filter.date as Record<string, string>).$lte = String(req.query.to);
  }
