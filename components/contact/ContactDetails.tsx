...
export default function ContactDetails() {
  ...
  const telHref = `tel:+91${CONTACT.phone}`;
        ...
        <div className={styles.block}>
          ...
          <div className={styles.hours}>
            <div className={styles.hoursGroup}>
              <strong>{CONTACT.hours.weekday.label}</strong>
              {CONTACT.hours.weekday.slots.map((slot) => (
                <span key={slot}>{slot}</span>
              ))}
            </div>
            <div className={styles.hoursGroup}>
              <strong>{CONTACT.hours.sunday.label}</strong>
              {CONTACT.hours.sunday.slots.map((slot) => (
                <span key={slot}>{slot}</span>
              ))}
            </div>
          </div>
